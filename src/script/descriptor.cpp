    ...
    if (ctx == ParseScriptContext::SP) {
        for (auto path : paths) {
            ...
            for (auto entry : path) {
                ...
                if (tmp_extkey.key.IsValid() && !extkey.Derive(tmp_extkey, entry)) {
                    error = strprintf("key '%s' is not valid", str);
                    return {};
                }
    ...
    if (ctx == ParseScriptContext::TOP && Func("sp", expr)) {
        ...
        for (auto& scan_pubkey : scan_pubkeys) {
            ...
            for (auto& spend_pubkey : spend_pubkeys) {
                CKeyID spend_keyId;
                if (auto root_pubkey{spend_pubkey->GetRootPubKey()}) {
                    spend_keyId = root_pubkey->GetID();
                } else if (auto root_extpubkey{spend_pubkey->GetRootExtPubKey()}) {
                    spend_keyId = root_extpubkey->pubkey.GetID();
                }
                if (spend_keyId.IsNull()) {
                    error = "sp(): could not get spend key ID";
                    return {};
                }
                if (scan_keys_rm.contains(spend_keyId)) {
                    // Do not remove this key because it is also a spend key
                    scan_keys_rm.erase(spend_keyId);
                }
