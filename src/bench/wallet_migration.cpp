...
static void WalletMigration(benchmark::Bench& bench)
{
    ...
    for (int i = 0; i < NUM_WALLETS; ++i) {
        ...
        auto database = MakeWalletDatabase(fs::PathToString(test_setup->m_path_root / filename), options, status, error);
    ...
    bench.epochs(NUM_WALLETS).run([&] {
        ...
        util::Result<MigrationResult> res = MigrateLegacyToDescriptor(fs::PathToString(test_setup->m_path_root / filename), "", context);
        assert(res);
        assert(res->wallet);
