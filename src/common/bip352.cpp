...
std::vector<secp256k1_xonly_pubkey> CreateOutputs(
    ...
    const std::vector<KeyPair>& taproot_keys,
    const COutPoint& smallest_outpoint
) {
    bool ret;
    ...
    for (size_t i = 0; i < taproot_keys.size(); i++) {
        taproot_keypair_ptrs.push_back((secp256k1_keypair *)taproot_keys[i].data());
    }
...
std::map<size_t, WitnessV1Taproot> GenerateSilentPaymentTaprootDestinations(const std::map<size_t, V0SilentPaymentDestination>& sp_dests, const std::vector<CKey>& plain_keys, const std::vector<KeyPair>& taproot_keys, const COutPoint& smallest_outpoint)
{
    bool ret;
