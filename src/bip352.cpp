...
ScanningData CreateScanningData(const SharedSecret& shared_secret, const CPubKey& receiver_spend_pubkey, const XOnlyPubKey& tx_output, uint32_t output_index)
{
    ...
    // Create the output and calculate the label data needed
    ret = secp256k1_silentpayments_receiver_create_scanning_data(secp256k1_context_static,
        &output_data, &label_data, shared_secret.begin(), &spend_pubkey_obj, output_index, &tx_output_obj);
    assert(ret);
    ret = secp256k1_xonly_pubkey_serialize(secp256k1_context_static, xonly_output.begin(), &output_data.P_output_xonly);
    assert(ret);
    // If the generated output is a match, return early without label data. The label data is not needed at this point,
    // and if they are a match, the label data is invalid anyways (tx_output - output = 0, the point at infinity).
    if (xonly_output == tx_output) return {xonly_output, uint256(output_data.t_k), {}, {}};
    ...
    return {xonly_output, uint256(output_data.t_k), label, label_negated};
}
...
std::pair<CPubKey, uint256> CreateLabelTweak(const CKey& scan_key, const int m) {
    ...
    bool ret = secp256k1_silentpayments_create_label_tweak(secp256k1_context_sign, &label, label_tweak, UCharCast(scan_key.data()), m);
    assert(ret);
    return {CPubKey{label.data}, uint256{label_tweak}};
}
