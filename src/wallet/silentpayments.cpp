...
std::optional<std::vector<uint256>> GetTxOutputTweaks(const BIP352::SharedSecret& shared_secret, CPubKey& spend_pubkey, std::vector<XOnlyPubKey> output_pub_keys, const std::map<CPubKey, uint256>& labels)
{
    ...
    do {
        ...
        output_pub_keys.erase(std::remove_if(output_pub_keys.begin(), output_pub_keys.end(), [&](XOnlyPubKey output_pubkey) {
            BIP352::ScanningData scanning_data = BIP352::CreateScanningData(shared_secret, spend_pubkey, output_pubkey, k);
            bool found = scanning_data.output == output_pubkey;
            if (!found) {
                auto it = labels.find(scanning_data.label);
                if (it == labels.end()) {
                    it = labels.find(scanning_data.label_negated);
                }
                if (it != labels.end()) {
                    scanning_data.t_k = BIP352::CombineTweaks(scanning_data.t_k, it->second);
                    found = true;
                }
            }
            if (found) {
                // Since we found an output, we need to increment k and check the vector again
                tweaks.emplace_back(scanning_data.t_k);
                keep_going = true;
                k++;
