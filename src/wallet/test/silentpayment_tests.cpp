...
BOOST_AUTO_TEST_CASE(bip352_send_and_receive_test_vectors)
{
    ...
    for (const auto& vec : tests.getValues()) {
        ...
        for (const auto& recipient : vec["receiving"].getValues()) {
            ...
            auto found_tweaks = GetTxOutputTweaks(shared_secret, sp_address.m_spend_pubkey, output_pub_keys, labels);
            ...
            for (const uint256& tweak : *found_tweaks) {
                ...
                uint256 raw_key_data(Span{UCharCast(spend_priv_key.begin()), UCharCast(spend_priv_key.end())});
                uint256 full_key_bytes = BIP352::CombineTweaks(raw_key_data, tweak);
                privkey.Set(full_key_bytes.begin(), full_key_bytes.end(), true);
